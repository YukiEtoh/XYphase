...
def variancehist(Variance):
    if Band == 'Band3':
       plt.hist(Variance,color='b',rwidth=1, bins=50)
    if Band == 'Band6':
       plt.hist(Variance,color='g',rwidth=1, bins=50)
    plt.savefig('oneScanVariance_allBaseBand'+Band+'.png')
    plt.xlabel('variance')
...
for Session in range(0,len(SessionList)):
    ...
    for BB_index in range(4):
        ...
        for ScanInterTime in range(0,len(scangapPoint)-1):       
            oneScanBB_var= np.var(XYimagPart[scangapPoint[ScanInterTime]:scangapPoint[ScanInterTime+1]])
            oneScanList= oneScanList + [oneScanBB_var] 
variancehist(oneScanList)
