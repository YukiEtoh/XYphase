...
def variancehist(Variance):
    ...
    plt.savefig('twoscanVariance_allBaseBand'+Band+'.png')
    plt.xlabel('variance')
...
for Session in range(0,len(SessionList)):
    ...
    for BB_index in range(4):
        ...
        for ScanInterTime in range(0,len(scangapPoint)-2):
            twoScanBB_var= np.var(XYimagPart[scangapPoint[ScanInterTime]:scangapPoint[ScanInterTime+2]])
            twoScanList= twoScanList + [twoScanBB_var]
variancehist(twoScanList)
